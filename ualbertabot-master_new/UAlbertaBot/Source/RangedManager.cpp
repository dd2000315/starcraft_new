...
int RangedManager::getAttackPriority(BWAPI::Unit rangedUnit, BWAPI::Unit target)
{
    ...
    if (rangedUnit->getType() == BWAPI::UnitTypes::Zerg_Hatchery)
    {
        return 100;//add
    }
    if (rangedUnit->getType() == BWAPI::UnitTypes::Zerg_Lair)
    {
        return 100;//add
    }
    if (rangedUnit->getType() == BWAPI::UnitTypes::Zerg_Hive)
    {
        return 100;//add
    }
    if (rangedUnit->getType() == BWAPI::UnitTypes::Zerg_Sunken_Colony)
    {
        return 101;
    }
