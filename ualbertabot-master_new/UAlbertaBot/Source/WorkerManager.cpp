...
void WorkerManager::handleIdleWorkers()
{
    ...
    for (auto & worker : m_workerData.getWorkers())
    {
        ...
        else if (m_workerData.getWorkerJob(worker) == WorkerData::Minerals && worker->isIdle())//add
        {
            setMineralWorker(worker);
        }
    }
...
void WorkerManager::setMineralWorker(BWAPI::Unit worker)
{
    ...
    //{
        ...
        //m_workerData.setWorkerJob(worker, WorkerData::Minerals, depot);
    //}
    //else
    //{
        //BWAPI::Broodwar->printf("No valid depot for mineral worker");
    //BWAPI::Broodwar->printf("setMineralWorker");
        for (auto& unit : BWAPI::Broodwar->self()->getUnits())
        {
            //if (unit->getType() == BWAPI::UnitTypes::Terran_Command_Center && unit->getPosition() != (Global::Bases().getPlayerStartingBaseLocation(BWAPI::Broodwar->self()))->getPosition())
            if (unit->getType() == BWAPI::UnitTypes::Terran_Command_Center)
            {
                BWAPI::Unitset resources = unit->getUnitsInRadius(300, BWAPI::Filter::IsResourceContainer);
                if (resources.empty())
                {
                    continue;
                }
                ...
                {
                    m_workerData.setWorkerJob(worker, WorkerData::Minerals, unit);
                }
                /*for (auto& res : resources)
                {
                    if (res)
                    {
                        worker->rightClick(res);
                        m_workerData.setWorkerJob(worker, WorkerData::Minerals, depot);
                    }
                }*/
...
BWAPI::Unit WorkerManager::getClosestDepot(BWAPI::Unit worker)
{
    ...
    for (auto & unit : BWAPI::Broodwar->self()->getUnits())
    {
        ...
        BWAPI::Unitset resources = unit->getUnitsInRadius(250, BWAPI::Filter::IsResourceContainer);
        if (resources.empty())
        {
            continue;
        }
