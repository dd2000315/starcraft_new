...
Squad::Squad(const std::string & name, SquadOrder order, size_t priority)//有可能是真正发出攻击或
    : m_name(name)
    , m_order(order)
...
void Squad::update()
{
    ...
    if (needToRegroup)
    {
        ...
        m_detectorManager.regroup(regroupPosition);//后加的
    }
    else // otherwise, execute micro
    {
        ...
        m_scienceVesselManager.execute(m_order);//后加的
        m_terranDropManager.update();//自己加的


        m_detectorManager.setUnitClosestToEnemy(unitClosestToEnemy());
        m_detectorManager.execute(m_order);
        m_comsatStationManager.execute(m_order);
        m_wraithManager.execute(m_order);
    }
}
...
void Squad::addUnitsToMicroManagers()
{
    ...
    BWAPI::Unitset scienceVesselUnits;//后加的
    BWAPI::Unitset terranDropUnits;//后加的
    BWAPI::Unit comsatStationUnits;
    BWAPI::Unitset wraithUnits;
    ...
    for (auto & unit : m_units)
    {
        if (unit->isCompleted() && unit->getHitPoints() > 0 && unit->exists())
        {
            ...
            {
                wraithUnits.insert(unit);
            }
            /*else if ((unit->getType() == BWAPI::UnitTypes::Terran_Dropship) || (unit->getType() == BWAPI::UnitTypes::Terran_Firebat))//后加的
            {
                terranDropUnits.insert(unit);
            }*/
            ...
            else if (unit->getType() == BWAPI::UnitTypes::Terran_Science_Vessel && !unit->getType().isBuilding())
            {
                detectorUnits.insert(unit);
            }
            ...
            else if (((unit->getType().groundWeapon().maxRange() > 32) || (unit->getType() == BWAPI::UnitTypes::Protoss_Reaver) || (unit->getType() == BWAPI::UnitTypes::Zerg_Scourge)) && !(unit->getType() == BWAPI::UnitTypes::Terran_Wraith))
            {
            ...
            /*else if (unit->getType() == BWAPI::UnitTypes::Terran_Science_Vessel)//后加的
            {
                scienceVesselUnits.insert(unit);
            }*/
            else if (unit->getType() == BWAPI::UnitTypes::Terran_Comsat_Station)
            {
                comsatStationUnits = unit;
            }
    ...
    m_terranDropManager.setUnits(terranDropUnits);

    m_scienceVesselManager.setUnits(scienceVesselUnits);
    m_comsatStationManager.setUnit(comsatStationUnits);
    m_wraithManager.setUnits(wraithUnits);
}
