...
const bool StrategyManager::shouldExpandNow() const
{
    ...
    if (Global::Workers().getNumIdleWorkers() > 8)
    {
        return true;
    }
...
bool StrategyManager::shouldExpandNow_custom()
{
    // if there is no place to expand to, we can't expand
    /*if (Global::Bases().getNextExpansion(BWAPI::Broodwar->self()) == BWAPI::TilePositions::None)
    {
...
const MetaPairVector StrategyManager::getBuildOrderGoal()
{
    ...
    {
        //return getTerranBuildOrderGoal();//改动处
        return getTerranBuildOrderGoal_custom();
    }
...
//const MetaPairVector StrategyManager::getTerranBuildOrderGoal_custom() const//原型没去掉const
const MetaPairVector StrategyManager::getTerranBuildOrderGoal_custom()//自己所改写的
{
    ...
    int numZerg_Lurker = UnitUtil::GetEnemyAllUnitCount(BWAPI::UnitTypes::Zerg_Lurker);
    ...
    //const BWAPI::Race race = BWAPI::Broodwar->enemy()->getRace();
    

    if (m_enemyRace == BWAPI::Races::Zerg)
    {
        m_strategyAppliyedTimes = m_strategyAppliyedTimes+1;
        BWAPI::Broodwar->drawTextScreen(430, 140, "BuildOrderGoal Applied Times %d", m_strategyAppliyedTimes);

        BWAPI::Broodwar->drawTextScreen(500, 60, "enemy race is Zerg.");
        if (Config::Strategy::StrategyName == "Terran_3RaxPressure")
        {
            ...
            goal.push_back(std::pair<MetaType, int>(BWAPI::UnitTypes::Terran_Vulture, numVultures+12));
            goal.push_back(std::pair<MetaType, int>(BWAPI::UnitTypes::Terran_Goliath, numGoliath+12));
            goal.push_back(std::pair<MetaType, int>(BWAPI::TechTypes::Tank_Siege_Mode, 1));
            goal.push_back(std::pair<MetaType, int>(BWAPI::UnitTypes::Terran_Siege_Tank_Tank_Mode, 12));


            /*if (numFactory >= 1)
            {
                goal.push_back(std::pair<MetaType, int>(BWAPI::UnitTypes::Terran_Vulture, 3));
            }

            if (numFactory >= 1 && numArmory == 1)
            {
                goal.push_back(std::pair<MetaType, int>(BWAPI::UnitTypes::Terran_Goliath, 3));
            }*/
            ...
            /*if (numMarines > 15)
            {
                ...
                if (numGoliath < 15 && numVultures < 15)
                {
                    goal.push_back(std::pair<MetaType, int>(BWAPI::UnitTypes::Terran_Goliath, 5));
                    goal.push_back(std::pair<MetaType, int>(BWAPI::UnitTypes::Terran_Vulture, 5));
                }
                //goal.push_back(std::pair<MetaType, int>(BWAPI::UnitTypes::Terran_Goliath, numGoliath + 3));
            }*/



            /*if (Global::MStrategy().detect_Zerg_Lurker() == true)
            {
                Global::MStrategy().getForLurkerBuildOrderGoal(goal);
            }*/
        }
    ...
    else if (m_enemyRace == BWAPI::Races::Protoss)
    {
        BWAPI::Broodwar->drawTextScreen(150, 60, "enemy race is Protoss");
        if (Config::Strategy::StrategyName == "Terran_3RaxPressure")
        {
            ...
            {
                //goal.push_back(std::pair<MetaType, int>(BWAPI::TechTypes::Tank_Siege_Mode, 1));
                goal.push_back(std::pair<MetaType, int>(BWAPI::UnitTypes::Terran_Medic, 2));
                //goal.push_back(std::pair<MetaType, int>(BWAPI::UnitTypes::Terran_Comsat_Station, 1));
    ...
    else if (m_enemyRace == BWAPI::Races::Terran)
    {
        BWAPI::Broodwar->drawTextScreen(60, 60, "enemy race is Terran");
    }

    if (shouldExpandNow())
    {
        goal.push_back(std::pair<MetaType, int>(BWAPI::UnitTypes::Terran_Command_Center, numCC + 1));
        //goal.push_back(std::pair<MetaType, int>(BWAPI::UnitTypes::Terran_SCV, numWorkers + 6));
    }
...
const MetaPairVector StrategyManager::getTerranBuildOrderGoal() const
{
    ...
    else if (Config::Strategy::StrategyName == "Terran_3RaxPressure")
    {
        goal.push_back(std::pair<MetaType, int>(BWAPI::UnitTypes::Terran_Marine, numMarines + 16));
